...
cd "$WORKSPACE_ROOT"
if "$PYTHON" "$ENGINE" run --cycle-type daily_research --trigger cron; then
  exit 0
else
  "$PYTHON" "$REPAIR" --cycle-type daily_research --latest-only || true
  "$PYTHON" "$NOTIFY" --cycle-type daily_research --mode failure --message "daily_research 执行失败，请查看 latest_status / cycle logs" || true
  exit 1
fi
