...
class SentimentAnalystWorker(WorkerBase):
    def __init__(self):
    ...
    def execute_task(self, task_dir: Path, task: dict) -> None:
        ...
        plan_text = result.get('text', '').strip() or 'No sentiment analysis plan returned.'
        artifacts.write_text('sentiment_plan.md', plan_text + '\n')
        candidate = {
            'task_id': task_dir.name,
            'objective': task.get('objective', ''),
            'mode': result.get('mode'),
            'model': result.get('model'),
            'sentiment_summary': plan_text.splitlines()[0] if plan_text else '',
            'target_files': normalized_request.get('target_files', []),
        }
        artifacts.write_json('sentiment_candidate.json', candidate)
        names = artifacts.list_artifacts()
        self.update_status(task_dir, 'artifact_ready', 70, 'artifacts_ready', artifacts=names)
        self.handoff_task(task_dir, 'test-expert', names, 'sentiment-analyst OpenCode 产物已生成，等待验收')


if __name__ == '__main__':
    worker = SentimentAnalystWorker()
