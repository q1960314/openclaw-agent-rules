...
# 正确初始化 Tushare API（参考 fetch_data_optimized.py 中的 init_pro 函数）
...
try:
    ...
    if hm_list_data is not None and len(hm_list_data) > 0:
        ...
        # 步骤 2: 从名录中提取游资名称
        print("\n[步骤 2/4] 从名录中提取游资名称...")
        if 'name' in hm_list_data.columns:
            dealer_names = hm_list_data['name'].dropna().unique().tolist()[:5]  # 取前 5 个
            log_test_result(
                '游资名称提取',
                'PASS',
                f'提取到 {len(dealer_names)} 个游资名称',
                f'示例：{dealer_names[:3]}'
            )
            ...
            for dealer_name in dealer_names[:2]:  # 测试前 2 个游资
                try:
                    detail_data = pro.hm_detail(dealer_name=dealer_name, 
                                               start_date=TEST_START_DATE,
                                               end_date=TEST_END_DATE)
                    
                    if detail_data is not None and len(detail_data) > 0:
                        log_test_result(
                            f'hm_detail 接口调用 ({dealer_name})',
                            'PASS',
            ...
            if detail_test_passed:
                log_test_result(
                    ...
                    'PASS',
                    '成功完成联动测试，数据关联性验证通过',
                    'hm_list (name) → hm_detail (dealer_name) 数据链完整'
                )
            else:
                log_test_result(
                    '联动测试 - 游资名录 + 游资明细',
...
print(f"\n执行 {stability_iterations} 次快速迭代测试...")

for i in range(stability_iterations):
    try:
        test_data = pro.ths_hot(date=TEST_DATE)
        if test_data is not None:
            print(f"[{i+1}/{stability_iterations}] ✅ 成功 (数据量：{len(test_data)})")
        else:
            print(f"[{i+1}/{stability_iterations}] ⚠️  空数据 (正常)")
        time.sleep(1)  # 1 秒间隔
    except Exception as e:
        stability_errors += 1
        print(f"[{i+1}/{stability_iterations}] ❌ 错误：{str(e)[:50]}")
...
print(f"错误率：{stability_errors / stability_iterations * 100:.2f}%")
...
if stability_errors == 0:
    ...
    log_test_result(
        '快速稳定性测试',
        'FAIL',
        f'完成 {stability_iterations} 次迭代，错误率 {stability_errors / stability_iterations * 100:.2f}%',
        '需要检查接口稳定性'
    )
