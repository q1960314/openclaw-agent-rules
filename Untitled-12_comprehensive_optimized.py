...
class Utils:
        ...
        for i in range(max_retry):
            ...
            try:
                ...
                if "token" in error_str or "积分" in error_str or "权限" in error_str:
                    logger.error(f"❌ Tushare致命错误：{e}，停止重试")
                    self.send_wechat_message(f"【Tushare错误】{str(e)[:200]}")
            ...
            try:
                with open(FETCH_PROGRESS_FILE, 'r', encoding='utf-8') as f:
                    ...
                    if (progress_data.get('fetch_type') == fetch_type 
                        ...
                        return progress_data.get('completed_stocks', [])
            except Exception as e:
    ...
    def check_api_permission(self):
        ...
        修复了之前把网络超时误判为积分不足的bug
        """
        logger.info("开始预检查Tushare接口权限...")
        permission_ok = True
...
def signal_handler(signum, frame):
    ...
    if FLASK_SERVER is not None:
        try:
            ...
            logger.info("✅ Flask服务已关闭")
        except Exception as e:
            logger.warning(f"Flask服务关闭异常：{e}")
    # 强制垃圾回收
    gc.collect()
    logger.info("✅ 程序已安全退出")
    sys.exit(0)
...
@flask_app.route('/')
def index():
    return f"量化系统后端API服务已启动！当前模式：{AUTO_RUN_MODE} | 当前策略：{STRATEGY_TYPE} | 扩展数据抓取：{FETCH_EXTEND_DATA}"
        ...
        try:
            ...
            try:
                ...
                if not utils.check_api_permission():
                    ...
                    utils.send_wechat_message("【数据抓取失败】接口权限检查失败，请检查Token和积分")
                ...
                with TASK_STATUS_LOCK:
                    ...
                    TASK_STATUS[task_id]['message'] = '开始并发抓取个股全量数据...'
                logger.info(f"开始并发抓取个股全量数据，线程数：{FETCH_OPTIMIZATION['max_workers']}")
                ...
                def fetch_single_stock(ts_code):
                    ...
                    try:
                        ...
                        if fetch_type == 'full' and ts_code in completed_stocks and os.path.exists(os.path.join(stock_dir, 'daily.csv')):
                            file_mtime = datetime.fromtimestamp(os.path.getmtime(os.path.join(stock_dir, 'daily.csv')))
                        ...
                        if df_daily.empty:
                            ...
                            return ts_code, False
                        df_daily = utils.price_adjust(df_daily, ts_code)
                        filepath = os.path.join(stock_dir, "daily.csv")
                        if os.path.exists(filepath) and fetch_type == 'latest':
                            existing_df = pd.read_csv(filepath, encoding='utf-8-sig', dtype={'trade_date': str})
                            df_daily['trade_date'] = df_daily['trade_date'].astype(str)
                            merged_df = pd.concat([existing_df, df_daily], ignore_index=True)
                        ...
                        df_daily_basic = utils.request_retry(pro.daily_basic, ts_code=ts_code, start_date=start_date_api, end_date=end_date_api)
                        if not df_daily_basic.empty:
                            df_daily_basic['trade_date'] = df_daily_basic['trade_date'].astype(str)
                            filepath = os.path.join(stock_dir, "daily_basic.csv")
                            if os.path.exists(filepath) and fetch_type == 'latest':
                                existing_df = pd.read_csv(filepath, encoding='utf-8-sig', dtype={'trade_date': str})
                                merged_df = pd.concat([existing_df, df_daily_basic], ignore_index=True)
                                merged_df = merged_df.drop_duplicates(subset=['trade_date'], keep='last')
                                merged_df.to_csv(filepath, index=False, encoding='utf-8-sig')
                            else:
                                df_daily_basic.to_csv(filepath, index=False, encoding='utf-8-sig')
                        ...
                        if FETCH_EXTEND_DATA:
                            ...
                            if not df_moneyflow.empty:
                                ...
                                df_moneyflow.to_csv(os.path.join(stock_dir, "moneyflow.csv"), index=False, encoding='utf-8-sig')
                            df_concept = utils.request_retry(pro.concept_detail, ts_code=ts_code, silent=True)
                            if not df_concept.empty:
                                df_concept.to_csv(os.path.join(stock_dir, "concept_detail.csv"), index=False, encoding='utf-8-sig')
                        ...
                        if EXTEND_FETCH_CONFIG.get('enable_top_list', False):
                            ...
                            if not df_top.empty:
                                ...
                                df_top.to_csv(os.path.join(stock_dir, "top_list.csv"), index=False, encoding='utf-8-sig')
                        ...
                        if EXTEND_FETCH_CONFIG.get('enable_finance_sheet', False):
                            ...
                            if not df_balance.empty:
                                ...
                                df_balance.to_csv(os.path.join(stock_dir, "balancesheet.csv"), index=False, encoding='utf-8-sig')
                            ...
                            if not df_cash.empty:
                                ...
                                df_cash.to_csv(os.path.join(stock_dir, "cashflow.csv"), index=False, encoding='utf-8-sig')
                            ...
                            if not df_income.empty:
                                ...
                                df_income.to_csv(os.path.join(stock_dir, "income.csv"), index=False, encoding='utf-8-sig')
                        ...
                        if EXTEND_FETCH_CONFIG.get('enable_hk_hold', False):
                            ...
                            if not df_hk.empty:
                                ...
                                df_hk.to_csv(os.path.join(stock_dir, "hk_hold.csv"), index=False, encoding='utf-8-sig')
                        ...
                        if EXTEND_FETCH_CONFIG.get('enable_cyq', False):
                            ...
                            if not df_cyq_chips.empty:
                                ...
                                df_cyq_chips.to_csv(os.path.join(stock_dir, "cyq_chips.csv"), index=False, encoding='utf-8-sig')
                            ...
                            if not df_cyq_perf.empty:
                                ...
                                df_cyq_perf.to_csv(os.path.join(stock_dir, "cyq_perf.csv"), index=False, encoding='utf-8-sig')
                        ...
                        if EXTEND_FETCH_CONFIG.get('enable_block_trade', False):
                            ...
                            if not df_block.empty:
                                ...
                                df_block.to_csv(os.path.join(stock_dir, "block_trade.csv"), index=False, encoding='utf-8-sig')
                        
                        with GLOBAL_LOCK:
                            if ts_code not in completed_stocks:
                                completed_stocks.append(ts_code)
                                if len(completed_stocks) % 100 == 0:
                    ...
                    try:
                        ...
                        if not limit_df.empty:
                            ...
                            if os.path.exists(limit_path) and fetch_type == 'latest':
                                existing_limit = pd.read_csv(limit_path, encoding='utf-8-sig', dtype={'trade_date': str})
                ...
                if EXTEND_FETCH_CONFIG.get('enable_top_list', False):
                    ...
                    if all_top_list:
                        ...
                        if os.path.exists(top_list_path) and fetch_type == 'latest':
                            existing_top = pd.read_csv(top_list_path, encoding='utf-8-sig', dtype={'trade_date': str})
                        ...
                        top_list_df.to_csv(top_list_path, index=False, encoding='utf-8-sig')
                    ...
                    try:
                        ...
                        if not top_inst_df.empty:
                            ...
                            if os.path.exists(top_inst_path) and fetch_type == 'latest':
                                existing_inst = pd.read_csv(top_inst_path, encoding='utf-8-sig', dtype={'trade_date': str})
                            ...
                            top_inst_df.to_csv(top_inst_path, index=False, encoding='utf-8-sig')
                    except Exception as e:
                ...
                if EXTEND_FETCH_CONFIG.get('enable_kpl_concept', False):
                    ...
                    try:
                        ...
                        if not concept_list_df.empty:
                            concept_list_df.to_csv(os.path.join(config['output_dir'], 'concept_list.csv'), index=False, encoding='utf-8-sig')
                        ...
                        if not concept_detail_df.empty:
                            concept_detail_df.to_csv(os.path.join(config['output_dir'], 'concept_detail.csv'), index=False, encoding='utf-8-sig')
                    except Exception as e:
                ...
                if EXTEND_FETCH_CONFIG.get('enable_index_weight', False):
                    ...
                    try:
                        ...
                        if not index_weight_df.empty:
                            ...
                            index_weight_df.to_csv(os.path.join(config['output_dir'], 'index_weight.csv'), index=False, encoding='utf-8-sig')
                    except Exception as e:
                ...
                if EXTEND_FETCH_CONFIG.get('enable_multi_news', False):
                    ...
                    if not df_news.empty:
                        ...
                        if os.path.exists(news_path) and fetch_type == 'latest':
                            existing_news = pd.read_csv(news_path, encoding='utf-8-sig')
                ...
                if EXTEND_FETCH_CONFIG.get('enable_stk_limit', False):
                    ...
                    if all_stk_limit:
                        ...
                        df_stk_limit.to_csv(stk_limit_path, index=False, encoding='utf-8-sig')
        ...
        try:
            ...
            if self.strategy_type == "打板策略":
                ...
                if 'is_first_board' in df_filter.columns and 'vol' in df_filter.columns:
                    ...
                    # 筛选缩量比例符合要求（1/3~1/2）、回调天数符合要求（3~10天）
                ...
                if 'board_high' in df_filter.columns and 'board_low' in df_filter.columns and 'close' in df_filter.columns:
                    ...
                    # 计算首板K线的支撑位：board_low + (board_high - board_low) * support_level
                    # 例：support_level=0.5 → 1/2分位；support_level=0.33 → 1/3分位
...
class BacktestSystem:
    def __init__(self, base_url):
        ...
        logger.info("✅ 回测系统初始化完成，完全基于Tushare真实数据，贴合A股实盘规则")
    ...
    def get_cached_data(self):
        ...
        if 'limit' in total_df.columns:
            # 【优化】未来函数修复：shift(2) 确保当日选股时看不到当日 limit 信息
    ...
    def run(self, df_total):
        ...
        logger.info(f"开始执行{self.strategy_display}策略回测，严格贴合A股实盘规则，无未来函数...")
        if 'trade_date' not in df_total.columns:
            logger.error("❌ 回测数据缺少trade_date列，无法执行！")
            return pd.DataFrame(), pd.DataFrame()
        trade_dates = sorted(df_total["trade_date"].unique())
        drawdown_stop_days = 0
        
        for idx, date in enumerate(tqdm(trade_dates, desc="回测进度")):  # 【优化】添加进度索引
            if not get_app_running():
                break
            try:
                ...
                for ts_code, pos in list(self.position.items()):
                    ...
                    # 【缩量潜伏策略专属】简化止损逻辑，避免board_low字段缺失问题
        ...
        if date_diff < 90:
            annual_return_desc = f"累计收益率{total_return_rate:.2f}%（回测周期不足3个月，不计算年化）"
...
class DailyStockPicker:
    ...
    def pick_stocks(self, df_latest, trade_date):
        ...
        rename_dict = {
            ...
            "total_score": "23维总评分", "score_detail": "评分明细", "open": "开盘价", "close": "收盘价",
...
def generate_param_optimize_suggestion(backtest_result):
    ...
    if date_diff < 90:
        annual_return_desc = f"累计收益率{total_return:.2f}%（回测周期不足3个月，不计算年化）"
...
"""
        ...
        for _, row in top3.iterrows():
            top3_text.append(f"├─ {row.get('股票代码', 'N/A')} {row.get('股票名称', 'N/A')} 评分：{row.get('23维总评分', 0)}")
...
# ============================================== 【本地数据完整性校验 - 完全保留】 ==============================================
def validate_data_consistency(df, required_columns, min_rows=0):
    """
    【优化 D3】数据一致性校验
...
def validate_local_data():
    ...
    if len(stock_dirs) < 5:
        logger.error(f"❌ 本地数据校验失败：个股数据不足（仅{len(stock_dirs)}只，建议至少5只）")
        return False
    
    sample_stocks = stock_dirs[:5]
    for stock_code in sample_stocks:
        daily_file = os.path.join(STOCKS_DIR, stock_code, 'daily.csv')
        if not os.path.exists(daily_file) or os.path.getsize(daily_file) < 1024:
            logger.error(f"❌ 本地数据校验失败：个股 {stock_code} 缺少有效 daily.csv")
            return False
...
def wait_for_api_ready(timeout=15):
    ...
    while time.time() - start_time < timeout:
        ...
        try:
            ...
            if response.status_code == 200:
                logger.info("✅ Flask API服务已就绪")
                return True
        except:
            pass
        time.sleep(0.5)
    logger.error("❌ Flask API服务启动超时，无法继续执行")
    return False
...
def run_by_mode():
    ...
    if AUTO_RUN_MODE == "仅服务":
        if is_port_in_use(SERVER_PORT):
            ...
            utils.send_wechat_message(f"【服务启动失败】端口{SERVER_PORT}已被占用，请更换端口或关闭占用程序")
            return
        logger.info("✅ 启动【纯API服务模式】，仅提供数据接口服务")
        utils.send_wechat_message(f"【服务启动成功】已启动纯API服务模式，服务地址：http://localhost:{SERVER_PORT}")
        print(f"服务运行中...按【Ctrl+C】停止服务")
        print("="*80)
        global FLASK_SERVER
        from werkzeug.serving import make_server
        FLASK_SERVER = make_server(SERVER_HOST, SERVER_PORT, flask_app)
        try:
        ...
        logger.info("✅ 启动【抓取+回测模式】，先执行数据抓取，完成后自动执行回测")
        utils.send_wechat_message(f"【抓取+回测启动】已启动抓取+回测模式，策略：{STRATEGY_TYPE}，开始抓取数据")
        if is_port_in_use(SERVER_PORT):
            logger.error(f"❌ 端口{SERVER_PORT}已被占用，请更换端口或关闭占用程序")
            return
        ...
        logger.info(f"数据抓取任务已提交，任务ID：{task_id}")
        print(f"全量数据抓取中...请稍候（任务ID：{task_id}），请勿关闭程序！")
        print("="*80)
        ...
        while True:
            ...
            if status.get('status') == 'completed':
                ...
                logger.error(error_msg)
                utils.send_wechat_message(f"【抓取+回测失败】{error_msg}")
                return
            else:
        ...
        logger.info("🎉 【抓取+回测模式】全流程执行完成！所有报告已导出至本地目录")
        print("="*80)
        print("抓取+回测全流程完成！结果文件：")
        print(f"1. 回测报告：一体化回测结果报告.xlsx")
        if VISUALIZATION:
            print(f"2. 可视化图表：charts/回测报告_{datetime.now().strftime('%Y%m%d')}.png")
        print(f"3. 抓取/运行日志：logs/quant_info.log")
        print(f"4. 本地数据存储：data/ & data_all_stocks/")
        print(f"5. 失败股票清单：data/failed_stocks.json（可下次自动补抓）")
        print("="*80)
        return
        ...
        if now.hour < 17 or (now.hour == 17 and now.minute < 30):
            logger.warning("当前时间过早，龙虎榜/涨跌停数据可能未更新，建议17:30后再试")
            utils.send_wechat_message(f"【每日选股提醒】当前时间（{now.strftime('%H:%M')}）过早，数据可能未更新，建议17:30后再试")
        ...
        logger.info("开始提交增量数据抓取任务（仅最新1天）...")
        task_id = str(uuid.uuid4())
        TASK_QUEUE.put({
            'id': task_id,
            'config': load_config(),
        ...
        logger.info(f"增量数据抓取任务已提交，任务ID：{task_id}")
        print(f"最新数据抓取中...请稍候（任务ID：{task_id}），请勿关闭程序！")
        print("="*80)
        ...
        logger.error(f"❌ 无效的运行模式：{AUTO_RUN_MODE}，仅支持：抓取+回测/仅服务/仅回测/每日选股")
        utils.send_wechat_message(f"【运行模式错误】配置的AUTO_RUN_MODE={AUTO_RUN_MODE}，仅支持「抓取+回测/仅服务/仅回测/每日选股」")
        return
...
def main():
    ...
    print("="*80)
    print("  量化系统 - 后端+回测+每日选股一体化服务【多资讯源集成最终版】")
    print("="*80)
    ...
    print(f"并发线程数：{FETCH_OPTIMIZATION['max_workers']} | 每分钟最大请求数：{FETCH_OPTIMIZATION['max_requests_per_minute']}")
