...
STOCK_PICK_CONFIG = {
    ...
    "fetch_days": 1                # 每日选股抓取天数（仅需最新 1 天，节省请求次数）
}
# ============================================ 【9. 后端核心配置 - 一般不用改】 ============================================
SERVER_HOST = "0.0.0.0"
SERVER_PORT = 5001
...
# 【优化】阶段 1 任务 2 参数更新：用户要求改为 1500 次/分钟
FETCH_OPTIMIZATION = {
    'max_workers': 20,            # 【可调】并发线程数，10000 分建议 15-20，根据你的积分调整
    'batch_io_interval': 10,      # 【可调】批量 IO 间隔（秒），默认 10 秒
    'max_requests_per_minute': 1500 # 【可调】每分钟最大请求数，用户要求 1500 次/分钟（10000 分高积分）
}
    ...
    while get_app_running():
        try:
            ...
            try:
                ...
                logger.info(f"开始并发抓取个股全量数据，线程数：4（优化后固定值）")
                ...
                def fetch_single_stock(ts_code):
                    ...
                    try:
                        ...
                        if os.path.exists(filepath) and fetch_type == 'latest':
                            ...
                            utils.save_to_parquet(merged_df, filepath)
                        else:
                            df_daily['trade_date'] = df_daily['trade_date'].astype(str)
                        ...
                        if not df_daily_basic.empty:
                            ...
                            if os.path.exists(filepath) and fetch_type == 'latest':
                                ...
                                utils.save_to_parquet(df_daily_basic, filepath)
...
def run_by_mode():
    ...
    if AUTO_RUN_MODE == "仅服务":
        if is_port_in_use(SERVER_PORT):
            ...
            return
        logger.info("✅ 启动【纯 API 服务模式】，仅提供数据接口服务")
        utils.send_wechat_message(f"【服务启动成功】已启动纯 API 服务模式，服务地址：http://localhost:{SERVER_PORT}")
